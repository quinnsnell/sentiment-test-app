"""Tests for the extract_json helper — a pure function that pulls JSON out
of possibly-messy LLM responses. No LLM needed for these tests.
"""
import pytest
...
def test_plain_json_passes_through():
    result = extract_json('{"sentiment": "positive", "confidence": 0.9}')
...
def test_json_embedded_in_prose():
    ...
    result = extract_json('Here is my analysis: {"sentiment": "neutral"} thanks!')
...
def test_raises_when_no_json():
    ...
    with pytest.raises(ValueError, match="no JSON"):
        extract_json("just some prose no braces here")
...
def test_raises_when_json_is_invalid():
    ...
    with pytest.raises(json.JSONDecodeError):
        ...
        extract_json('{"sentiment": positive}')
